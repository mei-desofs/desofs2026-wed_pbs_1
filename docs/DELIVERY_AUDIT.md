...
application-level backup encryption, real reset-token delivery, privileged-user
MFA, authenticated/deep DAST and full agent-based IAST. These must stay
...
| High | MFA for admins is not implemented. | No MFA controllers/services/tests; ASVS docs already mark out of scope. | Important ASVS gap if claimed as complete. | No, invasive | Keep as explicit limitation; do not mark MFA controls compliant. |
...
- Production TLS, SIEM/WORM, log retention, MFA and reset-token delivery are
