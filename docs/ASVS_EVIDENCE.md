...
| V6 Authentication | Partially Compliant | BCrypt, inactive-user checks, login rate limiting, logout audit and audit/security events | `LoginRateLimitSecurityTest`, `AdminUserManagementSecurityTest`, `RuntimeSecurityEventLoggingTest` | No MFA, password reset, password history/reuse checks or mature account recovery. |
...
| --- | --- | --- | --- | --- |
| JUnit/MockMvc | 109 tests after session-management additions; CI artifact still contains the downloaded pre-change run | `Deliverables/Phase 2/Evidence/testing/ci-surefire-test-reports (1)` plus local Maven output | Coverage gaps in admin/case/backup branches | Evidence accepted; upload a fresh CI artifact after push. |
