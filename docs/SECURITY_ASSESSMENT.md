...
| Error handling | `GlobalExceptionHandler`, Spring Security error responses, `ErrorHandlingSecurityTest`. | Stack traces/internal details are not exposed in tested flows and error responses include correlation IDs. | Correlation IDs are not yet propagated into structured application logs. | Implemented baseline |
| Browser security headers | `SecurityConfig`, `SecurityHeadersTest`, ZAP reports. | CSP, frame protection, referrer policy, permissions policy and cross-origin isolation headers are configured. | CSP still allows inline script/style until the static frontend is refactored. | Implemented baseline |
