...
| V8 Authorization (ASVS 5) | Field-level authorization | Analyst queue entries redact descriptions until assignment; auditor closed-case views omit descriptions, tracking hashes and notes; admin user DTOs omit password hashes | `ReportService`, `AuditReadService`, `UserService`, `AuditorAuthorizationTest`, `AdminUserManagementSecurityTest` | Implemented |
...
| V16 Logging and Monitoring | Mandatory security events | Login success/failure, logout, invalid JWT, forbidden access, upload rejection, report status change and unexpected errors are recorded as audit logs or security alerts | `AuditLogService`, `SecurityMonitoringService`, `AuthService`, `AuthController`, `JwtAuthenticationFilter`, `ReportService`, `RuntimeSecurityEventLoggingTest`, `BusinessLogicWorkflowSecurityTest` | Implemented |
| V16 Logging and Monitoring | Correlation and UTC timestamps | `X-Correlation-ID` is propagated to responses and stored with audit/security events; timestamps are created using UTC | `CorrelationIdFilter`, `AuditLog`, `SecurityAlert`, `GlobalExceptionHandler`, `RuntimeSecurityEventLoggingTest` | Implemented |
| V16 Logging and Monitoring | Secret redaction and event integrity | Event details redact passwords, bearer tokens, reset tokens, authorization values and tracking codes; audit logs and alerts include SHA-256 integrity hashes | `SecurityLogSanitizer`, `AuditLogService`, `SecurityMonitoringService`, `RuntimeSecurityEventLoggingTest` | Implemented |
